...
{
  $key = $_ENV['PEXELS_API_KEY'] ?? getenv('PEXELS_API_KEY');

  if ($key) {
    if (!defined('PEXELS_API_KEY')) {
      define('PEXELS_API_KEY', $key);
    }
  } else {
    error_log("PEXELS_API_KEY is not set.");
  }
}
...
//i know what you are gonna say use an api for this but remember you can always make your own website 
